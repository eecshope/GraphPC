...
class Node:
    def __init__(self, node: tree_sitter.Node, father=None):
        ...
        for child in node.children:
            if child.is_named and child.type != "comment" and child.type != "preproc_arg":
                self.is_leaf = False
...
def simulate_data_flow(node: Node, text: list, table: List):
    ...
    if node.node.type == "function_definition":
        if node.direct_next[1].node.type != "function_declarator":
            raise ValueError(f"No type declaration of function {node.get_text(text)}")
        ...
        for i, c in enumerate(node.node.children):
            ...
            if text[sp:ep] == ")":
                break
            """
            if Node._get_text(c.start_point, c.end_point, text) == ")":
                break
        meta = meta[2:-1]
...
def convert_ast_to_dgl(root: tree_sitter.Node, code: str, vocab: Dict, add_data=False):
    root = Node(root)
    named_nodes = list([])
    code = code.strip().replace("\t", " ").split('\n')

    if add_data:
        simulate_data_flow(root, code, [dict({})])
    ...
    def build_dgl(node, non_name_node_idx):
        ...
        if add_data:
            for vp in node.last_read:
                u.append(node.idx)
                v.append(vp.idx)
                edge_type.append(LAST_READ)

            for vp in node.last_write:
                u.append(node.idx)
                v.append(vp.idx)
                edge_type.append(LAST_WRITE)

            for vp in node.computed_from:
                u.append(node.idx)
                v.append(vp.idx)
                edge_type.append(COMPUTED_FROM)

        for child in node.node.children:
            if not child.is_named:
                u.append(node.idx)
                v.append(non_name_node_idx)
                edge_type.append(DIRECT_LINK)
                local_dict[non_name_node_idx] = Node._get_text(child.start_point, child.end_point, code)
                non_name_node_idx += 1
    ...
    pos_edge_type = torch.tensor(edge_type)
    uu = torch.cat([u, v], dim=0)
    vv = torch.cat([v, u], dim=0)
    neg_edge_type = pos_edge_type + EDGE_KIND if add_data else pos_edge_type + 1
