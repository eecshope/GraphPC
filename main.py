...
def main():
    ...
    parser.add_argument("--vocab_size", default=10631, help="size of the vocabulary")
    parser.add_argument("--n_features", default=256, help="dimension of the features")
    parser.add_argument("--n_classes", default=104, help="number of classes")
    ...
    trainer = pl.Trainer(gpus=1, max_epochs=20, terminate_on_nan=True, default_root_dir=args.root_dir,
                         auto_lr_find=True)

    if args.task == "test":
        print("Test not ready")
    else:
        trainer.fit(model, train_dataloader=GraphDataLoader(train_dataset, batch_size=32, shuffle=True),
